...
def leaching_requirement(ecw_ds_m: float, ece_threshold_ds_m: float) -> float:
    """
    Leaching requirement LR, fraction.

    Rhoades (1974) high-frequency form:  LR = ECw / (2 * ECe_max)
    ...
    if ecw_ds_m <= 0 or ece_threshold_ds_m <= 0:
        return 0.0
    return min(LR_CAP, ecw_ds_m / (2.0 * ece_threshold_ds_m))
...
def operating_hours_check(shifts: int, set_time_h: float, interval_d: float,
    ...
    if not positive:
        ...
        reason = (f"The cycle needs {required:.2f} h against {available:.2f} h "
                  "available. Reduce the number of shifts or shorten the interval.")
    else:
        reason = ""
