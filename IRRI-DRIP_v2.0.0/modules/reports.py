...
def show(ctx):
    ...
    if not stage_guard(S, "cost", "Cost Estimation", page_key="cost"):
        return
    context_strip(S)
...
def _exports(S, checks, vd):
    ...
    with pd.ExcelWriter(xbuf, engine="openpyxl") as xw:
        ...
        pd.DataFrame([
            ...
            {"Assumption": "Leaching requirement", "Value": "Rhoades high-frequency form"},
    ...
    with c2:
        ...
        snapshot = json.dumps({"irri_drip_project": "2.0.0", "S": S}, indent=1,
...
def _network_kml(S, stem, tag):
    ...
    kml = KML.network_kml(stem, gps, laterals,
                          ...
                          summary=f"IRRI-DRIP 2.0 network: {op['n_subunits']} subunits, "
                                  f"{op['shifts']} shifts.")
