...
def requirement_tab(S):
    ...
    lr = K.leaching_requirement(ecw, ece)
    kr = K.ground_cover_reduction_factor(kd["gc"])
    ...
    if res["peak_only"]:
        ...
        season_vol = etc / ea * setup["area_ha"] * 10.0 * season_days
        monthly = None
    ...
    if lr_class != "none":
        ...
        banner(kind, f"Leaching requirement {lr*100:.0f} % (ECw {ecw} dS/m against an ECe "
                     f"threshold of {ece} dS/m). {lr_msg}")
    ...
    if st.button("💾 Save", type="primary", key="save_water"):
        ...
        save(S, "water", {
            ...
            "ecw": ecw, "ece": ece, "lr": lr, "rain_method": rain_method,
