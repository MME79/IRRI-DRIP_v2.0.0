...
def show(ctx):
    ...
    if not stage_guard(S, "pump", "Pump Selection", page_key="pump"):
        return
    context_strip(S)
    ...
    with tabs[0]:
        ...
        ed = stable_editor("cost_rates", lambda: pd.DataFrame(
            [{"Rate": k, "Value": float(v)} for k, v in base_rates.items()]),
        ...
        caption("Pipe prices for manifolds and mainline come from the pipe catalogue "
                "(data/pipes.json) the sizing used; edit that file to change them, so the "
                "sizing and the price stay consistent.")
        if st.button("↺ Restore catalogue rates", key="cost_reset"):
            reset_editor("cost_rates", pd.DataFrame(
                [{"Rate": k, "Value": float(v)} for k, v in ctx["rates"]["rates"].items()]))
            st.rerun()
